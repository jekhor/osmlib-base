...
require File.join(File.dirname(__FILE__), '..', 'lib', 'OSM', 'Database')
require 'test/unit'
...
class TestReXml < Test::Unit::TestCase
    ...
    def test_tags
        tags = OSM::Tags.new
        tags['foo'] = 'bar'
        tags.to_rexml(@doc.root)
        out = @doc.to_s

        rexml = REXML::Document.new(out)
        element = REXML::XPath.first(rexml, '/osm/tag')
        assert_kind_of REXML::Element, element
        assert_equal 'tag', element.name
        assert_equal 'foo', REXML::XPath.first(rexml, '/osm/tag/@k').value
        assert_equal 'bar', REXML::XPath.first(rexml, '/osm/tag/@v').value
    end
    ...
    def test_multi_tags
        ...
        # Output should NOT contain entities
        assert_no_match /#/, out

        rexml = REXML::Document.new(out)
        elements = REXML::XPath.match(rexml, '/osm/tag')
    ...
    def test_node_empty
        node = OSM::Node.new
        node.to_rexml(@doc.root)
        out = @doc.to_s

        rexml = REXML::Document.new(out)
        element = REXML::XPath.first(rexml, '/osm/node')
        assert_kind_of REXML::Element, element
        assert_equal 'node', element.name
        assert_equal node.id.to_s, REXML::XPath.first(rexml, '/osm/node/@id').value
    end
    ...
    def test_node_with_data
        ...
        assert_kind_of REXML::Element, element
        assert_equal 'node', element.name
        assert_equal '45', REXML::XPath.first(rexml, '/osm/node/@id').value
        assert_equal 'user', REXML::XPath.first(rexml, '/osm/node/@user').value
        assert_equal '2007-12-12T01:01:01Z', REXML::XPath.first(rexml, '/osm/node/@timestamp').value
        assert_equal '10.0', REXML::XPath.first(rexml, '/osm/node/@lon').value
        assert_equal '20.0', REXML::XPath.first(rexml, '/osm/node/@lat').value
    end
    ...
    def test_node_with_tags
        ...
        keys = []
        REXML::XPath.each(rexml, '/osm/node/tag') do |element|
            assert_kind_of REXML::Element, element
            assert_equal 'tag', element.name
    ...
    def test_way
        ...
        way << OSM::Node.new(42)
        way << OSM::Node.new(43)
        way.to_rexml(@doc.root)
        out = @doc.to_s

        rexml = REXML::Document.new(out)
        element = REXML::XPath.first(rexml, '/osm/way')
        assert_kind_of REXML::Element, element
        assert_equal 'way', element.name
        assert_equal 'nd', REXML::XPath.first(rexml, '/osm/way/[1]').name
        assert_equal 'tag', REXML::XPath.first(rexml, '/osm/way/[3]').name
        assert_equal '4', REXML::XPath.first(rexml, '/osm/way/@id').value

        keys = []
        REXML::XPath.each(rexml, '/osm/way/tag') do |element|
            assert_kind_of REXML::Element, element
            assert_equal 'tag', element.name
        ...
        refs = []
        REXML::XPath.each(rexml, '/osm/way/nd') do |element|
            assert_kind_of REXML::Element, element
    ...
    def test_member
        ...
        member.to_rexml(@doc.root)
        out = @doc.to_s

        rexml = REXML::Document.new(out)
        element = REXML::XPath.first(rexml, '/osm/member')
        assert_kind_of REXML::Element, element
        assert_equal 'member', element.name
        assert_equal 'node', REXML::XPath.first(rexml, '/osm/member/@type').value
        assert_equal '18',   REXML::XPath.first(rexml, '/osm/member/@ref').value
        assert_equal 'foo',  REXML::XPath.first(rexml, '/osm/member/@role').value
    end
    ...
    def test_relation
        ...
        relation.to_rexml(@doc.root)
        out = @doc.to_s

        rexml = REXML::Document.new(out)
        element = REXML::XPath.first(rexml, '/osm/relation')
        assert_kind_of REXML::Element, element
